...
fun TacticalDebriefDialog(
    ...
    val titleText = when (status) {
        ...
        SimulationStatus.DEAD_PNX -> "DECESSO PER PNX IPERTESO"
        SimulationStatus.DEAD_MULTIPLE -> "DECESSO COMBINATO"
        else -> "DEBRIEFING SCENARIO"
    }

    val isTorso = scenario?.woundType == WoundType.TORSO

    val narrativeDescription = when (status) {
        SimulationStatus.SAVED -> {
            if (isTorso) {
                "MISSIONE COMPIUTA: Il soccorritore militare ha eseguito con successo tutte le manovre salvavita per ferita penetrante al torso prima dello scadere dei timer critici.\n\n" +
                "• Vie Aeree: Pervietà garantita (tempo residuo: ${uiState.formattedAirwayTimer}).\n" +
                "• PNX: Trattamento/Decompressione eseguita (tempo residuo: ${uiState.formattedPnxTimer}).\n\n" +
        ...
        SimulationStatus.DEAD_SUFFOCATED ->
            "CRITICITÀ LETALE: Il tempo limite di 7 minuti (07:00) per il controllo delle vie aeree è scaduto prima che venisse garantita la pervietà.\n\n" +
        ...
        SimulationStatus.DEAD_PNX ->
            "CRITICITÀ LETALE: Il tempo limite di 10 minuti (10:00) per il trattamento del pneumotorace (PNX) è scaduto.\n\n" +
            "La ferita penetrante al torso ha causato un pneumotorace iperteso con collasso polmonare, compressione dei grossi vasi mediastinici, shock ostruttivo e decesso sul terreno."

        SimulationStatus.DEAD_MULTIPLE ->
            "CRITICITÀ TOTALE: Più parametri vitali prioritari TCCC sono falliti contemporaneamente.\n\n" +
            "Il paziente è deceduto per la combinazione letale di complicanze acute multiple non trattate in tempo utile."
                    ...
                    Surface(
                        ...
                        Column(modifier = Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            ...
                            Row(
                                ...
                                Text("Pneumotorace (PNX):", style = MaterialTheme.typography.labelSmall, color = TextMuted)
                                Text(if (sc.pnxState == PnxState.PRESENTE) "PRESENTE (Torso)" else "Assente", style = MaterialTheme.typography.bodySmall, color = if (sc.pnxState == PnxState.PRESENTE) TacticalRedBright else TextPrimary, fontWeight = FontWeight.Bold)
                            }
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                horizontalArrangement = Arrangement.SpaceBetween
                            ) {
                                Text("Parametri Vitali:", style = MaterialTheme.typography.labelSmall, color = TextMuted)
                                Text("${sc.vitalSigns.formattedHr} | PA ${sc.vitalSigns.formattedBp} | SpO2 ${sc.vitalSigns.formattedSpo2}", style = MaterialTheme.typography.bodySmall, color = TacticalAirwayBlue)
                            }
                        }
                    }
                }
